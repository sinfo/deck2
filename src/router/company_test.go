...
func TestAddBilling(t *testing.T){
	...
	newCompany, err = mongodb.Companies.AddParticipation(newCompany.ID, credentials.ID, apd)
	assert.NilError(t, err)

	var f = false
	var employer = primitive.NewObjectID()
	var value = 10
	var invoiceNumber = "1"
	var emission = time.Now()
	var notes = "Some notes"

	csd := mongodb.CreateStatusData{
		Invoice: &f,
		Paid: &f,
		ProForma: &f,
		Receipt: &f,
	...
	b, errMarshal := json.Marshal(cbd)
	assert.NilError(t, errMarshal)

	res, err := executeRequest("POST", "/companies/"+newCompany.ID.Hex()+"/billing", bytes.NewBuffer(b))
	...
	billing, err := mongodb.Billings.GetBilling(updatedCompany.Participations[0].Billing)
	assert.NilError(t, err)

	assert.Equal(t, billing.Event, Event1.ID)
	assert.Equal(t, billing.Employer, employer)
	assert.Equal(t, billing.Value, value)
	assert.Equal(t, billing.InvoiceNumber, invoiceNumber)
	assert.Equal(t, billing.Emission.Sub(emission).Seconds() < 10e-3, true)
	assert.Equal(t, billing.Notes, notes)
