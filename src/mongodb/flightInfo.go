...
func (f *FlightInfoType) CreateFlightInfo(data CreateFlightInfoData) (*models.FlightInfo, error) {
	...
	insertResult, err := f.Collection.InsertOne(f.Context, c)

	if err != nil {
		log.Fatal(err)
	}

	if err := f.Collection.FindOne(f.Context, bson.M{"_id": insertResult.InsertedID}).Decode(&createdFlightInfo); err != nil {
...
func (f *FlightInfoType) DeleteFlightInfo(flightInfoID primitive.ObjectID) (*models.FlightInfo, error) {
	...
	deleteResult, err := FlightInfo.Collection.DeleteOne(f.Context, bson.M{"_id": flightInfoID})
	if err != nil {
		return nil, err
	}
