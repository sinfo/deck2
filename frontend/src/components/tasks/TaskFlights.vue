<template>
  ...
  >
    ...
    <div class="space-y-4">
      <!-- Needs flights selector -->
      <div class="space-y-2">
        <Label class="text-sm font-medium"
          >Does the speaker need flights?</Label
        >
        <Select v-model="needsFlights">
      ...
      <!-- Flight details (only when needs flights = yes) -->
      <template v-if="needsFlights === 'yes'">
        <Separator />
        ...
        <div class="space-y-3">
          ...
          <div class="grid grid-cols-2 md:grid-cols-4 gap-3">
            ...
            <div class="space-y-1">
              <Label for="arrival-date" class="text-xs"
                >Date (arrival LIS)</Label
              >
              <DatePicker v-model="arrivalDate" placeholder="Pick a date" />
            </div>
            <div class="space-y-1">
              <Label for="arrival-time" class="text-xs"
                >Time (arrival LIS)</Label
              >
              <Input
                id="arrival-time"
                v-model="arrivalTime"
        ...
        <div class="space-y-3">
          ...
          <div class="grid grid-cols-2 md:grid-cols-4 gap-3">
            ...
            <div class="space-y-1">
              <Label for="departure-date" class="text-xs"
                >Date (from LIS)</Label
              >
              <DatePicker v-model="departureDate" placeholder="Pick a date" />
            </div>
            <div class="space-y-1">
              <Label for="departure-time" class="text-xs"
                >Time (from LIS)</Label
              >
              <Input
                id="departure-time"
        ...
        <div class="space-y-3">
          ...
          <div class="grid grid-cols-2 md:grid-cols-4 gap-3">
            ...
            <div class="space-y-1">
              <Label for="flight-booking-ref" class="text-xs"
                >Booking Ref</Label
              >
              <Input
...
// Needs flights selector
type NeedsFlights = "not_responded" | "yes" | "no";
const needsFlights = ref<NeedsFlights>(
  (props.speakerTasks?.flights?.needsFlights as NeedsFlights) ??
    "not_responded",
);
...
function emitFlights() {
  emit("update:flights", {
    needsFlights: needsFlights.value,
...
const isComplete = computed(() => {
  if (needsFlights.value === "no") return true;
  if (needsFlights.value === "yes") return flightStatus.value === "bought";
  return false;
});
