import type { Speaker, SpeakerWithContactObject } from "@/dto/speakers";
...
export const getVariablesFromType = <T extends VariablesInput>(
  ...
  const vars: AnyEmailVariableInput[] = [
    ...
    createEmailVariable.memberArticle(
      input.member?.contactObject?.gender || "OTHER",
    ),
  ...
  if (isSpeakerVariablesInput(input)) {
    ...
    return [
      ...
      createEmailVariable.speakerArticle(
        (input.speaker as SpeakerWithContactObject)?.contactObject?.gender ||
          "OTHER",
      ),
      createEmailVariable.speakerSuffix(
        (input.speaker as SpeakerWithContactObject)?.contactObject?.gender ||
          "OTHER",
      ),
    ];
